          ...
          <div class="box">
            ...
            <div class="box-body">
               ...
               <thead>
                <tr>
                  ...
                  <th></th>
					...
					function getEmpName($id){
							$SQL_name=mysql_query("SELECT * FROM  emptbl WHERE id='$id'  ");
					
							while($crow = mysql_fetch_array($SQL_name)){
									$cid= $crow['id'];
									$empName= $crow['fname']." ".$crow['mname']." ".$crow['lname'];
									return $empName;
					...
					$SQL_=mysql_query("SELECT * FROM  leavecredits  ");
					
							while($crow = mysql_fetch_array($SQL_)){
									$cid= $crow['id'];
									$empid=getEmpName($crow['emp_id']);
									$leavecredits	= $crow['leavecredits'];
									 
									  ?>
				 <tr >
				
             	 <td><?php echo $empid; ?> </td>
				 <td><?php echo $leavecredits; ?> </td>
				 <td><a class="btn_edit_credit" id="<?php echo $cid; ?>"><button href="#editModal<?php echo $cid; ?>" role="button" data-toggle="modal" type="button" class='btn btn-success btn-xs'>Edit Leave Credits</button></a> </td>
						...
						<div id="editModal<?php echo $cid; ?>" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
								...
								<form class="form" method="post" action="operation/update_credit.php">
								<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
								<h3>Remaining Leave Credits</h3>
								</div>
								<div class="modal-body">
								<div class="row">
								<div class="col-sm-12">
									<input type="hidden" name="id"  value="<?php echo $cid; ?>"/>
									<input class="form-control" type="text" name="leavecredits"  value="<?php echo $leavecredits; ?>"/>
				...
				<form action="operation/update_maternity.php" method="post">
				<div class="col-sm-6">
				<br/>
				<?php
					$SQL_=mysql_query("SELECT * FROM  maternityleave  ");
					
							while($crow = mysql_fetch_array($SQL_)){
									$cid= $crow['id'];
									
									$maternity	= $crow['maternity_credits'];
							}
				?>
				<label>Maternity Leave</label>
									<input type="hidden" name="id"  value="<?php echo $cid; ?>"/>
										<input class="form-control" type="text" name="maternity_credits"  value="<?php echo $maternity; ?>"/><br/>
											<input type="submit" name="saveBtn" value="SAVE DETAILS" class="btn btn-primary btn-sm" >
								</div>
						</form >		
            </div>
