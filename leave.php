			...
			<table id="accountsData" class="table table-bordered table-striped" data-role="datatable" data-searching="true">
                ...
                <tr>
				  ...
				  <th></th>
					...
					$name=$name."<br/>	".getEmpname($row['empid']);
					...
					$SQL_=mysql_query("SELECT  * FROM  leavetbl   WHERE  approval='$mid'   ");
							...
							while($crow = mysql_fetch_array($SQL_)){
									...
									$date1=$crow['from_'];
									$date2=$crow['to_'];
									$approvalID=$crow['approval'];
									$approval=getEmpname($approvalID);
									
									$clear=getPartialApp($cid);
				...
				<?php if($status==""){
				?>
				<a href="operation/approve.php?id=<?php echo $cid; ?>&empid=<?php echo $empid; ?>&credit=<?php echo $countDays; ?>&appid=<?php echo $approvalID; ?>"><button type="button" class='btn btn-success btn-xs'>Approve</button></a>
				<a href="operation/reject.php?id=<?php echo $cid; ?>"><button type="button" class='btn btn-warning btn-xs'>Reject</button></a>
